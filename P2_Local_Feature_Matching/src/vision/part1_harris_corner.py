...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    h = filter.shape[0] // 2
    w = filter.shape[1] // 2
    padded = np.pad(img, ((h, ), (w, )), 'constant', constant_values = 0)
...
def nms_maxpool_pytorch(
    ...
    one = torch.ones(R.shape)
    
    R = torch.unsqueeze(torch.Tensor(R), 0)
    max_pool = nn.MaxPool2d((ksize, ksize), padding = ksize//2, stride = 1)
    median = torch.median(R)

    threshold = torch.where(R< median, zero, R)
    pooled = max_pool(threshold)

    binarize = torch.where(R == pooled, one, zero)
    keep = torch.squeeze(torch.mul(binarize, R))
    confidences, temp = torch.topk(keep.flatten(), k)
    x, y = np.array(np.unravel_index(temp.numpy(), keep.shape))
...
def get_harris_interest_points(
    ...
    if len(c) > k:
        ...
        y = y[:k]
        c = c[:k]
    # raise NotImplementedError('`get_harr
    # is_interest_points` function in ' +
    #     '`part1_harris_corner.py` needs to be implemented')
