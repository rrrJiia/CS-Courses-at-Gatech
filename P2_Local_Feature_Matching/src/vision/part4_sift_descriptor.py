...
def get_magnitudes_and_orientations(
    ...
    i, j = Ix.shape
    orientations = np.zeros((i, j))
    magnitudes = np.sqrt(np.square(Ix) + np.square(Iy))

    for ii in range(i):
        for jj in range(j):
            #calculating angles of gradients and swapping 
            orientations[ii, jj] =  np.arctan2(Iy[ii,jj], Ix[ii,jj])
...
def get_gradient_histogram_vec_from_patch(
    ...
    for i in range(4):
        for j in range(4):
            ...
            prev_j = 4*j
            latter_j = 4 * (j+1)
            mag_room = window_magnitudes[prev_i:latter_i, prev_j:latter_j]
            ori_room = window_orientations[prev_i:latter_i, prev_j:latter_j]
            histogram = np.histogram(ori_room, bins = 8, range = (-np.pi-(1e-6), np.pi), weights = mag_room)
...
def get_feat_vec(
    ...
    #############################################################################
    # print(c, r)
    c_prev = c-(feature_width-1)//2
    c_latter = c+feature_width//2+1
    r_prev = r-(feature_width-1)//2
    r_latter = r+feature_width//2 +1
    #print(c_prev)
    #print(c_latter)
    #print(r_prev)
    #print(r_latter)
    mag_grid = magnitudes[r_prev:r_latter, c_prev:c_latter]
    ori_grid = orientations[r_prev:r_latter, c_prev:c_latter]
