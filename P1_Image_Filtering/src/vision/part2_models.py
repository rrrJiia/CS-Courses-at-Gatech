...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        raw_np_kenel = create_Gaussian_kernel_2D(cutoff_frequency)
    ...
    def forward(
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        my_filter = self.get_kernel(int(cutoff_frequency.item()))
        low_frequencies = self.low_pass(image1, my_filter)
        high_frequencies = image2 - self.low_pass(image2,my_filter)
        image = low_frequencies + high_frequencies
        hybrid_image = torch.clamp(image, 0, 1)
        # raise NotImplementedError(
