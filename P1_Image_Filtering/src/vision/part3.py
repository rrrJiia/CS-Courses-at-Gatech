...
def my_conv2d_pytorch(image: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    (one, d1, h1, w1) = image.shape
    (N, temp_d1_group, k, k) = kernel.shape
    
    curr_groups = d1//temp_d1_group
    padding = k // 2
    
    d2 = N

    filtered_image = F.conv2d(image, kernel, padding = kernel.shape[2] // 2, groups = curr_groups, stride =1)
